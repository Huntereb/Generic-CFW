...
void setFirmMode(int mode){ //0 : SysNand, 1 : EmuNand
	...
	if(FileOpen(&firm, "tools/data/0004013800000002.bin", 0)){
		FileWrite(&firm, mode ? &nat_emuwrite : &mmc_original, 8, 0xCCF2C);
		FileWrite(&firm, mode ? &nat_emuread : &mmc_original, 8, 0xCCF6C);
		FileWrite(&firm, cfwmode_label, 4, 0x7A5A0);
		FileClose(&firm);
	}
}

int cfwMode(int mode){	//0 : SysNand, 1 : EmuNand
	setFirmMode(mode);
	File myFile;
	u8* native_firm = (u8*)0x21000000;
	if(FileOpen(&myFile, "tools/data/0004013800000002.bin", 0)){
...
void cfwModeSys(){
    ConsoleInit();
    ConsoleSetTitle("Booting CFW on SysNand...");
    print("Loading...\n"); ConsoleShow();
	cfwMode(0);
	print("Could not boot into SCFW.\n\nPress A to exit\n"); ConsoleShow();
	WaitForButton(BUTTON_A);
}

void cfwModeEmu(){
	if(!checkEmuNAND()) cfwModeSys();
	else{
		ConsoleInit();
		ConsoleSetTitle("Booting CFW on EmuNand...");
		print("Loading...\n"); ConsoleShow();
		cfwMode(1);
		print("Could not boot into CFW.\n\nPress A to exit\n"); ConsoleShow();
		WaitForButton(BUTTON_A);
	}
}
