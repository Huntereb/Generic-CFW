...
void DecryptTitleKeys() {
	...
	FileOpen(&dump, "tools/decTitleKeys.bin", 1);
...
/** This decrypts the encTitleKeys.bin inside tools directory on SD. */
void DecryptTitleKeyFile(void) {
	...
	UINT br = 0;
	u8 step = 0;
	print("Opening tools/encTitleKeys.bin...\n"); ConsoleShow();
	//decTitleKeys.bin that generated from other stuff can be handled streamly.
	FileOpen(&dump, "tools/decTitleKeys.bin", 1);
	rr = f_open(&dump, "tools/decTitleKeysA.bin", FA_WRITE | FA_CREATE_ALWAYS);
	if (rr != FR_OK) {
		print("FAIL open tools/decTitleKeysA.bin [%04X].\n", rr);
		f_close(&dump);
		print("\nPress A to exit\n"); ConsoleShow();
		WaitForButton(BUTTON_A);
		return;
	}
	rr = f_open(&tick, "tools/encTitleKeys.bin", FA_READ | FA_OPEN_EXISTING);
	if (rr != FR_OK) {
		f_close(&tick); f_close(&dump);
		print("FAIL open tools/encTitleKeys.bin [%04X].\n", rr);
		print("\nPress A to exit\n"); ConsoleShow();
		WaitForButton(BUTTON_A);
		return;
	}
