...
#define FILEPACK_OFF	0x140000			//Offset in Launcher.dat
#define FILEPACK_SIZE	0x95000
...
void LoadPack(){
	...
	File FilePack; FileOpen(&FilePack, "cfw.dat", 0);
	...
	for(int i = 0; i < nEntry; i++){
		...
		if(!CheckHash(Entry[i].off, Entry[i].size, Entry[i].hash)){
			DrawString(TOP_SCREEN, " cfw.dat is corrupted!", 0, 240-8, BLACK, WHITE); 		//Who knows, if there is any corruption in our files, we need to stop
			while(1);
		}
	}
}
...
int CheckHash(unsigned char* file, unsigned int size, unsigned int hash){
	unsigned int HASH = 0;
